...
# === Radice pazienti ===
CARTELLA_PAZIENTI = "pazienti"
os.makedirs(CARTELLA_PAZIENTI, exist_ok=True)
...
class SchedaPazienteWindow(QWidget):
    def __init__(self, nome, cognome, eta,
        ...
        # === Cartella paziente ===
        self.cartella_paziente = os.path.join(
            CARTELLA_PAZIENTI, f"{self.nome}_{self.cognome}_{self.eta}a"
        )
        os.makedirs(self.cartella_paziente, exist_ok=True)

        self.data_file = os.path.join(self.cartella_paziente, "dati.json")
        self.cartella_report_completi = os.path.join(self.cartella_paziente, "report_completi")
        self.cartella_report_indici = os.path.join(self.cartella_paziente, "report_indici")
        self.cartella_grafici = os.path.join(self.cartella_paziente, "grafici")

        os.makedirs(self.cartella_report_completi, exist_ok=True)
        os.makedirs(self.cartella_report_indici, exist_ok=True)
        os.makedirs(self.cartella_grafici, exist_ok=True)

        # carica dati se esiste il file dati.json
        ...
        self.elimina_btn.setStyleSheet("background:#e53935; color:white; font-weight:bold; font-size:15px; padding:12px 22px")
        ...
        self.nuova_valutazione_btn.setStyleSheet("background:#43a047; color:white; font-weight:bold; font-size:15px; padding:12px 22px")
    ...
    def apri_cartella_paziente(self):
        apri_cartella(self.cartella_paziente)
...
def apri_cartella(path_cartella):
    if platform.system() == "Windows":
        os.startfile(path_cartella)
    elif platform.system() == "Darwin":  # macOS
        subprocess.Popen(["open", path_cartella])
    else:  # Linux
        subprocess.Popen(["xdg-open", path_cartella])
